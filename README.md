...
- Phase 3 已完成：Public Evaluator MVP——L0/L1 评测记录四 family、replay runner、scorer 四级、统计三类、六门 hard gates、meta-tests、首批公开 benchmark suites（详见 v0.4.0 tag 与 Phase 3 验收报告）。后续 CR4 以 additive `evaluation-attempt/v1` / `evaluation-result/v1` 关闭失败留档缺口，同时逐字保持 `evaluation-run/v1`；旧 `compare()` 的观测单位问题尚未修复
