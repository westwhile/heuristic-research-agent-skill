...
- `ml-evidence/v2` → `ml-evidence-v2.schema.json`（L4 final experiment successor：kind 固定 experiment_run，显式携带最终评估 partition 与 split pin；train/validation 不安全值保持 schema 合法，由 adapter 语义层拒绝；非实验 assessment/audit evidence 继续使用冻结 v1）
