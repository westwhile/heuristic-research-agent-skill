"""Exact-archive gate for controlled PyTorch/CUDA interruption recovery.

Run from an exported commit with the PyTorch-enabled interpreter.  The caller
supplies Git commit/tree object IDs and the archive SHA-256 independently
resolved before extraction.  The gate starts one source child, confirms its
atomically committed checkpoint, terminates only that owned child through its
``Popen`` object, and compares fresh-process recovery with an uninterrupted
control.  Stable output excludes timestamps, timing, PIDs, and local paths.
"""
...
def _fixture() -> dict:
    return {
        ...
        "fixture_id": "exact-archive-cuda-controlled-interruption-001",
        "case_sha256": canonical_sha256(
            {
                "case": "exact-archive-cuda-controlled-interruption-001",
...
def _manifest(runtime: dict, fixture: dict) -> DLRunManifest:
    ...
    payload.update(
        {
            "manifest_id": "exact-archive-controlled-interruption-manifest-001",
            "run_id": "exact-archive-controlled-interruption-run-001",
            "study_id": "exact-archive-controlled-interruption-study-001",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="dl-interruption-gate-") as temp:
        observation = run_pytorch_controlled_interruption_recovery(
            manifest, fixture, temp
        )
    ...
    stable_projection = {
        ...
        "processes": [
            {"role": row["role"], "completed_steps": row["completed_steps"]}
    ...
    print(
        "DL CONTROLLED INTERRUPTION GATE: PASS "
    ...
    print(
        "DL CONTROLLED INTERRUPTION RESULT: "
