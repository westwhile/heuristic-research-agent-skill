...
- `verify_p7c3_math_agent_smoke.py`：只从 exact commit/archive 运行两个公开 Math case 的 baseline/Candidate 四进程 Codex smoke；Candidate 仅投影到仓库外临时 `.agents/skills/`，固定 ephemeral/read-only/no-web/no-retry，并分别核验 launcher、`thread.started` session 与 `turn.completed`；公开 evidence 只保留 hash、计数、verdict 和无原文的逐指标 score vector，不保存 raw session、stderr、JSONL、最终输出或本机路径。
