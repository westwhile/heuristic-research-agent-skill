...
- 计划版本：v5.31-phase7-p7d6-candidate-proposal
- 初次制定日期：2026-08-18
- 状态同步日期：2026-08-30
- 仓库：`westwhile/heuristic-research-agent-skill`
- 本地工作树：`$PROJECT_ROOT`（由操作者在本机配置，不写入公开绝对路径）
- 当前状态：Phase 0—5 已逐层验收发布；Apache-2.0 OSS-R0 治理、`0.6.1` 包元数据与 annotated `v0.6.1` source Release 已完成。O5 公开试用入口已合入但仍等待维护者选择参与者，当前无外部结果；O6 仅准备公开证据草案，私密字段和最终提交未启动。Phase 6 L1–L4 与 R1—R6A 已依次合入；R6B 继续冻结为 `TARGET_FROZEN / ZERO_EXTERNAL_SUBMISSIONS`。Phase 7 P7A、Correctness Reset CR1—CR8、P7B1—P7B4、P7C1—P7C3 与 P7D1A/P7D1B 已依次合入。P7D2/P7D3 的六个同一操作者公开 Math baseline 会话仍分别为 3 个通过、2 个通过与 1 个 `capture_inconclusive`；PR #62 已以 fail-closed Windows/POSIX 进程树隔离关闭所暴露的工程清理缺口，但不改变原 verdict。P7D4 的结构化输出兼容批次从 exact main 执行 4 个全新、零重试会话并得到 4 个 exact-oracle `qualified_failure`，共享 `graph_structure` 机制；P7D5 再以三个不同 task template 获得两个五维 lineage 无碰撞的同机制 failure package，形成仓库外正式 Pattern。P7D5 的一次真实 Candidate 调用因合同不合格被拒绝且未重试。P7D6A/6B 由 PR #64 加固稳定脱敏诊断并完成 deterministic contract calibration；随后用全新 proposal ID 恰好调用一次真实 Candidate generator，得到一个 byte-closed `proposal_ready` Candidate，并仅在仓库外通过官方 `skill-creator` 物化和 `quick_validate` 为 `MATERIALIZED_UNINSTALLED_CANDIDATE`。当前上限为 `P7D6_REAL_CANDIDATE_PROPOSAL_READY / MATERIALIZED_UNINSTALLED_CANDIDATE / ZERO_REAL_INDEPENDENT_REVIEWS / ZERO_INDEPENDENT_FORWARD_ACCEPTANCES / ZERO_HIDDEN_EVALUATIONS / ZERO_PROMOTIONS`；未证明 Candidate 行为改善、真实科研质量、外部采用或 lifecycle authority。未创建 `v0.7.0` Tag/Release，Skill 安装、激活与发布 Gate 未开启。
...
P7B3 实施终态（2026-08-26）：Ready PR #48 的 exact head `f8c3100e84ddf372566bb25eed2ce0a6f9868e4d` 以 merge commit `ebef9906ef6f8b6ab19341faf52f35306e4bce5b` 合入，两者 tree 均为 `0a71c0c049d7efe2d5a20aff0a2a5c4a6dcd41b4`。exact archive 为 Python 3.12.13/3.14.5 各 1100/1100（各 6 个预期 skip），SHA-256 `97b6588948f3102ddb8887d9e2eec5d103c3d29a296524d862eba58373c29c6f`；两项 clean-install、82% branch coverage、两层 Ruff、31-file mypy 和 Windows governance 通过。本机 CUDA compatibility Gate 稳定投影为 `bb1dbe0bbee8bdcc62677ac75782d5615608e5861392582fbb5370d477567561`，未写 receipt。PR CI run `32874713519` 与 exact merge SHA 的 main run `32875093337` 均四项 jobs、两个 Windows governance 成功。P7B3 落地 `skill-static-validation-receipt/v1` 与单一 `validate_skill_candidate` seam；`static_pass` 不证明真实 Router/Agent 执行、语义审查、Skill 物化/安装或 Promotion。

P7B4 实施终态（2026-08-26）：Ready PR #49 的 exact head `8a3de612a446593d3f7f817bcc5bebc26d236794` 以 merge commit `0bfa324ae2ceeeb88932e3cbf8142729fb0aa733` 合入，两者 tree 均为 `83febe661cc9adff106625116a1520ed076d2e12`。exact archive 为 Python 3.12.13/3.14.5 各 1107/1107（各 6 个预期 skip），SHA-256 `20249b36e4c3e179e7f61f5b518413fe90b479e2c81deece48ff1d7b269eb25d`；两项 clean-install、82% branch coverage、两层 Ruff、32-file mypy 和 Windows governance 通过。本机 CUDA compatibility Gate 稳定投影为 `99008c89b6f7b7d9cc9093ebf8e3fcf466c245b326af61330aecbefe9ab3293a`，未写 receipt。PR CI run `32877471634` 与 exact merge SHA 的 main run `32877878739` 均四项 jobs、两个 Windows governance 成功。P7B4 落地 `skill-semantic-review-attestation/v1` 与单一 `attest_skill_semantic_review_protocol` seam；不同 reviewer/session 字符串和 `shared_context=false` 只是声明标签，合成 `protocol_accept` / `protocol_reject` 不构成真实独立审查。状态固定为 `P7B4_SEMANTIC_REVIEW_PROTOCOL_READY / ZERO_REAL_INDEPENDENT_REVIEWS / ZERO_REAL_SKILL_EXECUTIONS / ZERO_PROMOTIONS`。

P7C1 实施终态（2026-08-26）：Ready PR #51 的 exact head `c39ba9a4e96b005f32ec3c868c07892e5aba8507` 以 merge commit `802bd23872ed31d73b09229745b54cbd34cc66c1` 合入，两者 tree 均为 `2a9c4e502f8ddfa5ac2510ddf7ea231fb0d2226a`。exact archive 为 Python 3.12.13/3.14.5 各 1115/1115（各 6 个预期 skip），SHA-256 `2b760a6a5b1e70b0ef5b54bce0b534cd123e8e760c62013249ace5c3596552d4`；两项 clean-install、81% branch coverage、两层 Ruff、34-file mypy 和 Windows governance 通过。本机 CUDA compatibility Gate 稳定投影为 `e9fa67042d6f183d49373d97f399950a2d64db3de6b75a1f4dface309037d7a1`，未写 receipt。PR CI run `32924685360` 与 exact merge SHA 的 main run `32924926788` 均四项 jobs、两个 Windows governance 成功。P7C1 落地单一 `run_skill_forward_test` 深接口和 deterministic in-process / constrained fixed-worker local-process 两个 adapter，复用既有 attempt/result、suite comparison、ArtifactRecord 与 envelope closure，没有新增重复 Core schema；所有 Math/Quant 接受、拒绝、超时和故障只属于 synthetic engineering。状态固定为 `P7C1_EXECUTION_SEAM_READY / ZERO_REAL_AGENT_EXECUTIONS / ZERO_REAL_INDEPENDENT_REVIEWS / ZERO_PROMOTIONS`；未运行真实 Codex/模型、未物化或安装 Skill、未打开 Phase 8 或发布 Gate。

P7C2 实施终态（2026-08-26）：Ready PR #53 的 exact head `c01199d6a489bfdd2c266ab0951ebf893d98ff39` 以 merge commit `d58fd610c75b08c167b4c7d9c7715e539a235ca5` 合入，两者 tree 均为 `ba5cf22ef238a53693bf1d68ea5bfb94e62892a8`。exact archive 为 Python 3.12.13/3.14.5 各 1123/1123（各 6 个预期 skip），SHA-256 `f42f72b86bd335aa275591fc965ed2c4e82a26335bed2c5cceac627c3eaf24e2`；两项 clean-install、81% branch coverage、两层 Ruff、35-file mypy 通过。本机 CUDA compatibility Gate 稳定投影为 `a2df03affa46ab9f82cae3cab7d8fa0b64c0554bcf997900053be6b280947566`，未写 receipt。PR CI run `32929597881` 与 exact merge SHA 的 main run `32929833971` 均四项 jobs、两个 Windows governance 成功。P7C2 落地单一 `run_skill_forward_suite` 深接口，在任何 adapter 调用前对完整 `case × seed` 网格全量 preflight，预注册 primary/guardrail、seed 与最坏重试预算；执行开始后保留所有失败 attempt，只有完整 result/run 网格才复用既有 `suite-comparison/v1`。Math accept、Quant reject 与 `2 cases × 2 seeds × 2 arms` 都是 synthetic engineering；状态固定为 `P7C2_PUBLIC_FORWARD_SUITE_READY / ZERO_REAL_AGENT_EXECUTIONS / ZERO_HIDDEN_EVALUATIONS / ZERO_PROMOTIONS`，未物化/安装 Skill，未打开 Phase 8 或发布 Gate。

P7C3 实施终态（2026-08-26）：Ready PR #55 的 exact head `59eb2e231e1b4fc9bbc2d33c37fc0897909cc4b2` 以 merge commit `247c4d21423f3393e36ddc20a3f941f19c509745` 合入，两者 tree 均为 `282f9f1b5b0168d95fcffdd854d5433cabc35717`。exact archive 为 Python 3.12.13/3.14.5 各 1139/1139（各 6 个预期 skip），SHA-256 `e79a9381484fbab7d4469c9b1fe8e644eff4985211c0e6a1783db4680b8e12db`；两项 clean-install、81% branch coverage、两层 Ruff、36-file mypy 通过。本机 CUDA compatibility Gate 稳定投影为 `73d43ab090633f90243f90650c7990a1cd7073af77c55cec10a3244a0052cf3f`，未写 receipt。该 exact archive 的最终新 smoke 以 `gpt-5.6-sol`、xhigh、read-only、approval-never、no-web、ephemeral 配置执行 explicit-load 与 declared-exclusion 两个公开 Math case 的 baseline/Candidate 两臂，4/4 launcher、4/4 互异 Agent sessions 与 4/4 completed turns 全部 PASS；清洗 evidence canonical SHA-256 为 `444e5ea7ff0bcd2ae2782e3d63a81b081a64163b22e3da19057c8bc1cd23b7d9`，文件 SHA-256 为 `1aee5329917f926cc81c00ef1e53d17eaa9885485d1bad38e6ef5a5e5b6dc088`，raw session/transcript/output/path 未入库，Candidate 未安装/激活且临时工作区已清理。PR CI run `32969508625` 与 exact merge SHA 的 main run `32969940539` 均四项 jobs、两个 Windows governance 成功。前三次 fail-closed launcher/label/runtime-oracle 尝试保留为仓库外审计历史且均未以重试覆盖；只有提交 `59eb2e2` 的新四进程 Gate 支持终态 `P7C3_REAL_AGENT_SMOKE_RECORDED / ZERO_INDEPENDENT_FORWARD_ACCEPTANCES / ZERO_HIDDEN_EVALUATIONS / ZERO_PROMOTIONS`。这不证明独立 fresh-session acceptance、统计改善、真实科研质量、外部采用或 lifecycle authority。

P7D1A/P7D1B 实施终态（2026-08-28）：Ready PR #57/#58 的 exact head `cf3b7458d4c05420920a99a262c3685ac14d8c44`、`b2d99e2ba88a543c871c3340aca11dc8625f3112` 分别以 merge commit `f80ee1b990fb86943c7493cfe16f44226fc66f5d`、`f80f3e536257dff20a66fdbd95d0bfad85b165d9` 合入；共享 tree 为 `651096a956e8f95093b55342eed5c95282c1ada2`、`e0d7cb10ececaf4ed8f171a760c1a163910d1341`，PR CI runs `32979271039`、`32983501047` 四项 jobs 全绿。P7D1A 落地 `capture_public_agent_failure`，真实 baseline 强制单会话、零重试，只允许 oracle-proven、privacy/cleanup 闭合的错误形成 qualified failure chain；P7D1B 落地 `propose_skill_candidate`，至少两个不同 lineage 共享可复用 Pattern 才可进入既有 eligibility/bundle/closure Gate。两批 synthetic fixtures 只证明合同，不是真实失败或 Candidate。PR #58 合入后的 main run `32983946454` 因 GitHub Actions check-suite control-plane 失同步被 Support 清理，不计作代码成功；fresh-run 恢复 PR #59 以 merge commit `561919750ed31260fa76d396e57e97d7c0e467a7` 合入，其精确 main run `33137010990` 四项 jobs与两个 Windows governance 成功。

P7D2 校准终态（2026-08-28）：从 commit `561919750ed31260fa76d396e57e97d7c0e467a7`、tree `afc45b4c7eedccee69de3e6031876ef45fb62da0`、archive SHA-256 `53c37852a1b3e0c7946d9f0e5917c62df59b3d80c4c77ea7c6a336538e15a3e5` 的 exact archive，按预注册顺序执行随机重标号 Mizzi-A、独立生成 1-2-3-A、随机重标号 Schur-A。每题只有一个 `gpt-5.6-sol` / xhigh / 30 分钟上限 / read-only / no-Web 会话，零重试；三题分别在 226、98、275 秒完成并通过独立 exact oracle，均为 `pass / no_failure`。由于不存在 qualified failure，预冻结第四槽未执行、Candidate Gate 未打开。batch Gate SHA-256 `2901e9284968c5263ec343effd80069fb9c51e8263a5a28fb11d587b9eb65430`，evidence index SHA-256 `b8ca24e1bfd6c79ef947447eb457721a52da9bca7680b5cd203598ef54fc76de`；raw transcript/output/path 未入库。状态固定为 `P7D2_PUBLIC_BASELINE_CALIBRATION_RECORDED / ZERO_QUALIFIED_FAILURES / ZERO_REAL_CANDIDATES / ZERO_HIDDEN_EVALUATIONS / ZERO_PROMOTIONS`。

P7D3 校准终态（2026-08-29）：从 commit `ad60f0b678615412fa9859df6c8e6a49e7a79a33`、tree `95d3311483568f519c33fbc972431c90fe7b03f2`、archive SHA-256 `dafff342235df24d770de500bcf07cd18a9ea09e08d86d8f919b5fd1ab302aed` 的 exact archive，预冻结三个两两非同构且非重标号的公开 line-graph 主案例及每个主案例的独立 sibling。独立 exact oracle 通过稳定集分拆、monomial 系数和完整 Kostka 有理反演计算，三个主案例目标系数依次为 740、0、276。每题只有一个 `gpt-5.6-sol` / xhigh / 30 分钟上限 / read-only / no-Web 会话且零重试：`frontier-b/c` 为 `pass / no_failure`；`frontier-a` 在 1800 秒执行超时后为 `capture_inconclusive`，Windows 后代进程继承管道又延迟了外层清理，因此该槽不能算模型失败或 qualified failure。后两槽使用只改变进程树清理的仓库外 watchdog；没有改变案例、顺序、oracle、模型、预算、评分或重试规则。qualified failure 为 0，第四槽未执行，Candidate 授权建议为 `not_supported_by_current_evidence`。预注册 SHA-256 `66f8fab2b4652a08c77050307ab8b8158b6f0c62af1594e1cf2156dc8ba0fa86`，安全附录 SHA-256 `84c135ed94c880b9774acdc99fb935ecd2a723f2dc5cf4895d572edf7e5dd0a1`，batch Gate SHA-256 `a8df6db3df1a110fda08840239b07d8d262a7d2f4353fa7eb44b0861653bf5f9`，evidence index SHA-256 `4b33ec3088fbc2b8b3f48842db6d6b833f742053e2920bb58e162f3f180bf85e`；44/44 索引工件复算一致，raw transcript/model output/path 未入库。该批当时的状态固定为 `P7D3_SCHUR_FRONTIER_CALIBRATION_RECORDED / ZERO_QUALIFIED_FAILURES / ZERO_REAL_CANDIDATES / EXECUTION_CONTAINMENT_HARDENING_REQUIRED / ZERO_HIDDEN_EVALUATIONS / ZERO_PROMOTIONS`。

P7D3 execution-containment hardening 终态（2026-08-30）：Ready PR #62 的 final head `17b48ab9ef912c6221381f4ed3798e87fa142989` 以 merge commit `968b70480c544fb22c202df602ca6459b3c6e6ff` 合入，两者 tree 均为 `a182a57225785679efeb2fa615470afe63a30205`。新增私有 `run_process_contained` 模块统一 spawn、bounded communicate、timeout、进程树终止、reap 与清理核验；POSIX 使用独立 process group，Windows 固定递归 descendants 并在 `taskkill /T /F` 后逐 PID fail-closed。`cleanup_failed` 与普通 timeout 分离，并阻断 P7D1A qualified failure 与 P7D1B Candidate proposal。exact archive SHA-256 `14ac38a61c0cc72faefe7d218f964081a209b16668c96e9097ed82369f96ef6e`，Python 3.12.13/3.14.5 各 1161/1161（各 6 个预期 archive skip），两项 clean-install、81% branch coverage、两层 Ruff、39-file Linux/Windows mypy 通过；本机 CUDA compatibility Gate 稳定投影 `3f1e121786931befddaae5bcc52fbba377cd4cb798e43fafaa24898b20713ffe`，未写 receipt。PR CI run `33284411205` 与 exact merge SHA 的 main run `33284605519` 四项 jobs、两个 Windows governance 全绿。该工程关闭不修改 P7D3 原始 calibration verdict，不证明敌对进程隔离、安全 sandbox、模型/科研质量、Candidate 改善或 lifecycle authority。

P7D4/P7D5 公开失败校准终态（2026-08-30）：P7D4 首批会话因没有结构化 final output 按规则停止为 `inconclusive`，未选择性重跑。完成服务端 schema 预检、对象化证书行和独立 `--output-last-message` smoke 后，以全新预注册和 case IDs 从 commit `8b443f6b994aa949b385364327b7c7c6e424577e`、tree `183b65a27ccfc05ebd7ec11d6a4609d7b8c5c156`、archive SHA-256 `3d882ea69d8a5f8bee009e372b5ced720bd93c469b190192d8c05e7a10d4c298` 启动四个 `gpt-5.6-sol` / xhigh / 30 分钟 / read-only / no-Web / zero-retry 会话；三个主案例与首个失败的预冻结 sibling 均为 exact-oracle `qualified_failure`，共享 `graph_structure` 机制。P7D4 batch Gate SHA-256 `9039aa1ec3be900aacf30c5846160d4f8df9084cf95b55759b02dfb5fe32d7d1`，evidence index SHA-256 `382770b83f3cbe9c6c6e48e7b0410a5ffe6721e20ef5d461ed73f47964180b37`。P7D5 随后执行三个不同公开 task template 的新会话，两个 qualified failures 在 `independence_group`、`origin_run_id`、`dataset_lineage_id`、`task_template_id`、`semantic_duplicate_group` 五维逐项无碰撞并共享同一机制，生成仓库外正式 `research-pattern/v1`（SHA-256 `55d1d4e027c629661f3fed74252b85318f102f0a261ff0bb06a2ce1cff2895ba`）。P7D5 batch Gate SHA-256 `c11731eaf6be63aaa683e9448e1c29531a14b7c3504ac616f7e2b1d1c70ef3bc`；一次授权 Candidate 调用以 `candidate_contract_invalid` 被拒绝，未生成 Candidate、未重试或覆盖该失败。

P7D6 Candidate 合同与提案终态（2026-08-30）：Ready PR #64 的 final head `6d28ba7ab020d2fdaabaea67cd8b2f3de4689883` 以 merge commit `ad3bd5226226eb751bffdb1c37ab15f799dc35a7` 合入，两者 tree 均为 `a3498346d33a969bd4d023beede40f3090fefbbb`。P7D6A 在既有 `propose_skill_candidate` 深接口中增加稳定、脱敏的 `failure_stage` / `failure_code`，未知 runner 异常统一为不回显原始输出的 `generation_runner_error`；P7D6B deterministic calibration 对 accept、缺字段、frontmatter 不一致与 restricted-content 路径全部得到预期 fail-closed 结果（SHA-256 `6d8bbcd914b77d0bbf04f87991d341133386d256dfcf9ef571accd991630ca30`）。工程 exact archive SHA-256 `aff3442a59a3150c9fb857ac4817ccefd1d3b7a32d7e8a411a690c2ccde8cd71`，Python 3.12.13/3.14.5 各 1161/1161（各 6 个预期 archive skip），两项 clean-install、81% branch coverage、两层 Ruff、39-file mypy 与本机 CUDA compatibility Gate 通过；PR CI run `33315574223` 和 exact merge SHA 的 main run `33315765885` 四项 jobs、两个 Windows governance 全绿。随后从该 exact main（archive SHA-256 `9c17493fabcb4013484b4f7493776ead4550fc1fe16ae1e7688b4f649daef2b1`）复用冻结 P7D5 Pattern/两份 failure packages，以全新 proposal `p7d6-schur-canonical-contract-proposal-ad3bd522` 恰好运行一次真实 generator；215.9 秒后得到 `proposal_ready`、byte closure true、零 blocker，generation-result SHA-256 `b31db017d549c9cb705347eb2799ec332a74e213383752372784d6e7c80877fb`，且没有持久化 raw model output。仅在该结果后，仓库外唯一临时目录用官方 `skill-creator` 初始化并以 PyYAML 6.0.3 运行 `quick_validate`，生成确定性 ZIP `verify-schur-certificates-8bdd0ef71e8169f6.zip`（SHA-256 `8bdd0ef71e8169f682418aad33bcb7fb660009ffe052f9c936ea315c31730239`；manifest SHA-256 `2443c910086cd504c6f5b7aafdaae4ef35b682514d358f9c5f975e08596b6e93`）。Candidate 未进入仓库或 `skills/staging/`，未安装、激活、发布、独立语义审查、fresh-session forward test、Hidden Evaluation 或 Promotion；物化和 quick validation 只证明结构。
...
4. （P7B3 静态部分已实施，ADR-0018）`agents/openai.yaml` 等平台元数据与 Skill payload 分层校验；Skill description 同时描述正触发与重要排除场景，并加入声明式 Router 负例；尚未执行真实 Router；
...
7. （P7B3 部分实施）运行静态验证、路径/密钥/引用扫描、trigger exact collision、重复名称、payload descriptor diff；公开 regression、真实 Router 和公开 dev A/B 尚未执行；
...
19. （P7B4 协议已实施，ADR-0019）语义审查协议绑定 exact Candidate/P7B3 receipt/review evidence bytes，记录 drafter/reviewer/static-validator 标签分离、session/model/independence-group/shared-context 声明与七项固定维度；所有 fixture 结论只允许 `protocol_*`，真实 independent semantic PASS 与 ready-for-private-review 仍未完成；
20. （P7C1 已实施，ADR-0020）用 Math/Quant synthetic fixtures 穿过单一 `run_skill_forward_test` seam，覆盖 deterministic in-process 与 constrained fixed-worker local-process adapters、mutation/deletion/timeout/fail-closed tests；exact Candidate、public case/evaluator artifacts、model/runner/scorer、Router mode、oracle 和 Skill-only axis 均冻结。既有 evaluation/closure records 已能表达本批事实，因此未新增 Core successor；真实 Agent、fresh-session、secure sandbox 与 Promotion 仍未完成。
21. （P7C2 已实施，ADR-0021）用单一 `run_skill_forward_suite` seam 复用 P7C1 两个 adapters，按稳定顺序展开完整 public synthetic `case × seed` 网格；所有 cell 在零执行时全量 preflight，primary/guardrail、seed 和最坏重试预算预注册，执行失败保留 attempt 且不得选择性排除，只有完整 result/run 网格才组装既有 `suite-comparison/v1`。真实 Agent/Codex、Candidate 物化、fresh-session、Hidden Evaluator 与 Promotion 仍未完成。
22. （P7C3 已实施，ADR-0022）用单一 `run_agent_skill_forward_trial` seam 在仓库外临时目录投影 exact Candidate，并以 deterministic adapter 固化 contract、以 Codex CLI adapter 完成同一操作者的公开 Math 四进程 smoke；`evaluation-attempt/result/run` 记录 launcher/session/turn、runtime-load oracle 与 cleanup 事实，provider seed 明确为空且不伪造 suite comparison。真实独立 reviewer/fresh-session acceptance、隐式 Router、Hidden Evaluator、统计改善、Skill 安装/激活与 Promotion 仍未完成。
23. （P7D1A 已实施，ADR-0023）用单一 `capture_public_agent_failure` seam 和 deterministic/Codex CLI adapters 捕获公开 baseline 失败；真实执行必须单会话、零重试，attempt 必须保留，只有独立 oracle 证明错误且脱敏、restricted-content screening 与 workspace cleanup 全部闭合时才生成 FailureObservation/Analysis/ResearchCase chain。通过、超时、runner error、non-oracle 或 privacy-open 终态均不伪造成 qualified failure。
24. （P7D1B 已实施，ADR-0024）用单一 `propose_skill_candidate` seam 和 deterministic/Codex CLI adapters 只从至少两个不同 lineage 的 qualified public failures 与共享可复用 Pattern 提案；继续复用 eligibility、skill-candidate bundle、artifact closure、受限内容与 cleanup Gate；
25. （P7D4/P7D5 已完成仓库外公开校准）从 exact main 与独立 exact oracle 获得同机制 qualified failures，再以不同 task template 和五维 lineage 无碰撞 Gate 形成正式 Pattern；这些仍是同一操作者公开证据，不替代独立 review 或 hidden suite；
26. （P7D6A/P7D6B 已实施，ADR-0026）Candidate proposal 失败以稳定 `failure_stage` / `failure_code` 脱敏分类，deterministic contract calibration 覆盖接受与三类拒绝；新 proposal ID 的一次真实调用已形成 byte-closed Candidate；
27. （P7D6 条件物化已完成于仓库外）官方 `skill-creator` 初始化与 `quick_validate` 仅生成 immutable、uninstalled Candidate package；静态/语义/forward/hidden review、安装、激活、发布与 Promotion 仍未完成。
