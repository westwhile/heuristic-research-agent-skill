...
- 计划版本：v5.30-phase7-p7d3-execution-containment-hardening
- 初次制定日期：2026-08-18
- 状态同步日期：2026-08-30
- 仓库：`westwhile/heuristic-research-agent-skill`
- 本地工作树：`$PROJECT_ROOT`（由操作者在本机配置，不写入公开绝对路径）
- 当前状态：Phase 0—5 已逐层验收发布；Apache-2.0 OSS-R0 治理、`0.6.1` 包元数据与 annotated `v0.6.1` source Release 已完成。O5 公开试用入口已合入但仍等待维护者选择参与者，当前无外部结果；O6 仅准备公开证据草案，私密字段和最终提交未启动。Phase 6 L1–L4 与 R1—R6A 已依次合入；R6B 继续冻结为 `TARGET_FROZEN / ZERO_EXTERNAL_SUBMISSIONS`。Phase 7 P7A、Correctness Reset CR1—CR8、P7B1—P7B4、P7C1—P7C3 与 P7D1A/P7D1B 已依次合入。P7D2/P7D3 共完成六个同一操作者的公开 Math baseline 会话：P7D2 为 3/3 `pass / no_failure`；P7D3 的两个 Schur frontier 主案例通过，一个在冻结的 1800 秒执行上限后为 `capture_inconclusive`。两批均为零重试、零 qualified failure、零 Candidate，适应性第四槽均未启用。该 inconclusive 暴露的 Windows 后代进程清理缺口已由 PR #62 以 fail-closed Windows/POSIX 进程树隔离工程关闭；这不改变原校准 verdict，也不形成安全 sandbox 结论。当前上限为 `P7D3_SCHUR_FRONTIER_CALIBRATION_RECORDED / P7D3_EXECUTION_CONTAINMENT_HARDENED_ENGINEERING_ONLY / ZERO_QUALIFIED_FAILURES / ZERO_REAL_CANDIDATES / ZERO_INDEPENDENT_FORWARD_ACCEPTANCES / ZERO_HIDDEN_EVALUATIONS / ZERO_PROMOTIONS`；未证明统计改善、真实科研质量、外部采用或 lifecycle authority。未创建 `v0.7.0` Tag/Release，Skill 安装、激活与发布 Gate 未开启。
...
24. （P7D1B 已实施，ADR-0024）用单一 `propose_skill_candidate` seam 和 deterministic/Codex CLI adapters 只从至少两个不同 lineage 的 qualified public failures 与共享可复用 Pattern 提案；继续复用 eligibility、skill-candidate bundle、artifact closure、受限内容与 cleanup Gate。synthetic accept/reject 不是真实 Candidate，Candidate 物化、静态/语义/forward/hidden review、安装、激活与 Promotion 仍未完成。
