...
- 当前状态：Phase 0—5 已逐层验收发布（`v0.1.0`/`v0.2.0`/`v0.3.0`/`v0.4.0`/`v0.5.0`；`v0.5.1` 为归档缺件 hotfix；Phase 5 为 `v0.6.0`）。Phase 5（ML Adapter）L1–L6 的发布提交为 `c72e31eb4d5dbd367b20f24678e94682b963fed9`。OSS-R0 PR-A—PR-D 已由 PR #14—#17 合入；release-prep PR #18 的 merge commit `5af73595f847702930e0c1966986f3d06d3c1c35` 通过 main CI run `32619619333` 四项 required jobs、Windows governance、双解释器 archive suite 与 archive install Gate。annotated `v0.6.1` Tag object `2cdb9621d05211c779f933836adae476241206c0` 已指向该提交，正式 GitHub Release 的六项 assets 已回下载并通过 SHA-256 对账。PR #19 的 merge commit `b9a3b8268575fe32399b83295595710944c6a772` 已把 O5 公开入口合入 `main`，对应 run `32622282409` 四项全绿；当前等待维护者选择外部参与者，仍无外部结果。O6 仅进入公开证据草案准备，私密字段和最终提交均未启动。Phase 6 L1–L4 已通过 PR #21 的 merge commit `6c63c0bad88f032fb1091cdc5c91242bf22b2087` 合入 `main`，上限为 synthetic engineering。Phase 6 R1 已通过 PR #23 的 merge commit `4c7b47e0ac1db26d76107dea836e8172b981e698` 合入 `main`；该 merge tree 与通过双解释器 981/981 archive suite、clean-archive install 和两进程 CUDA 稳定哈希 Gate 的 PR head tree `26d187489df798bad0fdb3663b41509ed61125f4` 相同，main push CI run `32684239392` 四项 jobs 与两个 Windows governance 步骤全绿。当前最高证据是单主机 PyTorch/CUDA bounded synthetic fixture 的 real-framework/hardware engineering observation；真实数据、driver/跨 GPU 复现、外部 checkpoint store、scheduler recovery、完整 nested-CV 训练、自动晋级闭环、可安装 Skill、生产和外部采用仍未验收。未创建 `v0.7.0` Tag 或 Release，发布 Gate 未开启。
