...
- 当前状态：Phase 0—4 已逐层验收发布（`v0.1.0`/`v0.2.0`/`v0.3.0`/`v0.4.0`/`v0.5.0`；`v0.5.1` 为归档缺件 hotfix，发布证据链见各 Phase 验收报告）；Phase 5（ML Adapter）进行中——ADR-0008 已接受，L2（数据合同）、L3（声明式泄漏检查）、L4（final-evaluation Gate + 合成 runner）与 L4.1（evidence→case 绑定、validated contract、声明/assignment 双 pin、诚实 IID 执行包络）已交付，L5—L6 未实施。尚无 group/time-series/nested 执行合同、真实 ML 执行器、数据验收、自动晋级闭环或生产发布能力。
...
- 标准库纯函数合成实验 runner（baseline/resource parity、seed 重复研究、确定性规范产物；零 I/O、零第三方依赖；L4/L4.1 已交付，只支持 contract-bound IID/no-transform/no-search 显式内存合成数据）；
