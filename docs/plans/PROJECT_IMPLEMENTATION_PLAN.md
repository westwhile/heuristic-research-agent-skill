...
- 当前状态：Phase 0—4 已逐层验收发布（`v0.1.0`/`v0.2.0`/`v0.3.0`/`v0.4.0`/`v0.5.0`；`v0.5.1` 为归档缺件 hotfix，发布证据链见各 Phase 验收报告）；Phase 5（ML Adapter）进行中——ADR-0008 已接受，L2—L6 未实施。尚无真实研究执行器、自动晋级闭环或生产发布能力。
