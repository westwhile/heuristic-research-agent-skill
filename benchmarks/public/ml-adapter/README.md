...
每个 locator 指向 `tests/fixtures/adapters/ml-case/v1/valid/` 中 schema 合法的 `ml-case/v1`。这里的“负例”指必须被 Adapter 语义 Gate 拒绝的载荷，而不是结构非法 JSON。目录记录原始文件 SHA-256；E2E 测试同时验证 locator 边界、hash、预期接受/拒绝结果和 rule ID。
